...
from utils_r.data_utils_back import normalize_pose
from utils_r.pose_utils_label import gen_clip_seg_data_np, get_ab_labels
...
class PoseSegDataset(Dataset):
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
        ...
        self.global_data, self.segs_score_np, self.segs_label_np = \
        ...
        if abnormal_train_path is not None:
            ...
            self.global_data_ab, self.segs_score_np_ab, self.segs_label_np= \
        ...
        self.num_samples, self.C, self.T, self.V = self.segs_data_np.shape ## 原本的
        # self.num_samples, self.T,self.C,  self.V = self.segs_data_np.shape ## 

        ##### 分开 01 正常和异常，取值的时候各取batchsize个
        indices_ab = np.argwhere(self.segs_label_np.sum(axis=1)> 0) # 有异常的
        indices = np.argwhere(self.segs_label_np.sum(axis=1) == 0)
        self.segs_data_np_ab = self.segs_data_np[indices_ab].squeeze()
        self.segs_label_np_ab = self.segs_label_np[indices_ab].squeeze()
        self.segs_data_np_norm = self.segs_data_np[indices].squeeze()
        self.segs_label_np_norm = self.segs_label_np[indices].squeeze()
        # del self.segs_data_np
        # del self.segs_label_np
        self.num_samples , self.C, self.T, self.V = self.segs_data_np_norm.shape
        # 将数组复制3次
        self.segs_data_np_ab = np.repeat(self.segs_data_np_ab, len(self.segs_data_np_norm) // len(self.segs_data_np_ab)+1, axis=0).squeeze()
        self.segs_data_np_ab = self.segs_data_np_ab[:len(self.segs_data_np_norm)].squeeze()
        self.segs_label_np_ab = np.repeat(self.segs_label_np_ab, len(self.segs_label_np_norm) // len(self.segs_label_np_ab) +1, axis=0).squeeze()
        self.segs_label_np_ab = self.segs_label_np_ab[:len(self.segs_label_np_norm)].squeeze()
    ...
    def __getitem__(self, index):
        ...
        if self.apply_transforms:
            ...
            data_numpy = np.array(self.segs_data_np_norm[sample_index])
            data_transformed_norm = self.transform_list[trans_index](data_numpy)
            data_numpy = np.array(self.segs_data_np_ab[sample_index])
            data_transformed_ab = self.transform_list[trans_index](data_numpy)
        else:
            sample_index = index
            data_transformed_norm = np.array(self.segs_data_np_norm[index])
            data_transformed_ab = np.array(self.segs_data_np_ab[index])
            trans_index = 0  # No transformations

        if self.normalize_pose_segs:
            data_transformed_norm = normalize_pose(data_transformed_norm.transpose((1, 2, 0))[None, ...],
                                              **self.args).squeeze(axis=0).transpose(2, 0, 1)
            data_transformed_ab = normalize_pose(data_transformed_ab.transpose((1, 2, 0))[None, ...],
                                              **self.args).squeeze(axis=0).transpose(2, 0, 1)

        ret_arr_norm = [data_transformed_norm, trans_index]
        ret_arr_norm += [self.segs_label_np_norm[sample_index]]
        
        ret_arr_ab = [data_transformed_ab, trans_index]
        ret_arr_ab += [self.segs_label_np_ab[sample_index]]
        # ######## great ########### 0706 ck
        # self.segs_data_np = self.segs_data_np
        # ####################

        # ret_arr += [self.segs_score_np[sample_index]]  # 不用这俩
        # ret_arr += [self.labels[sample_index]]
        # ret_arr += [self.segs_label_np[sample_index]]

        return [ret_arr_norm,ret_arr_ab] # 
...
def gen_dataset(person_json_root, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True,
    ...
    # json_list = sorted([fn for fn in dir_list if fn.endswith('tracked_person_label.json')])
    json_list = sorted([fn for fn in dir_list if fn.endswith('tracked_person_label.json')], key = lambda x: (x.split('_')[0], x.split('_')[1]))
    ...
    for person_dict_fn in tqdm(json_list):
        ...
        clip_segs_data_np, clip_segs_meta, clip_keys, single_pos_np, _, score_segs_data_np, label_segs_data_np= gen_clip_seg_data_np(
        ...
        segs_label_np.append(label_segs_data_np)
    ...
    segs_label_np = np.concatenate(segs_label_np, axis=0)
    ...
    if ret_global_data:
        if ret_keys:
            return segs_data_np, segs_meta, person_keys, global_data_np, global_data, segs_score_np,segs_label_np
        else:
            return segs_data_np, segs_meta, global_data_np, global_data, segs_score_np,segs_label_np
    if ret_keys:
        return segs_data_np, segs_meta, person_keys, segs_score_np,segs_label_np
    else:
        return segs_data_np, segs_meta, segs_score_np, segs_label_np
