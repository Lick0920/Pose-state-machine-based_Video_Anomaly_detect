...
def gen_clip_seg_data_np(clip_dict, start_ofst=0, seg_stride=4, seg_len=12, scene_id='', clip_id='', ret_keys=False,
    ...
    for idx in sorted(clip_dict.keys(), key=lambda x: int(x)):
        sing_pose_np, sing_pose_meta, sing_pose_keys, sing_scores_np, sing_label_np = single_pose_dict2np(clip_dict, idx)
        if dataset == "UBnormal":
        ...
        curr_pose_segs_np, curr_pose_segs_meta, curr_pose_score_np, curr_pose_label_np = split_pose_to_segments(sing_pose_np,
                                                                                            sing_pose_meta,
                                                                                            sing_pose_keys,
    ...
    if len(pose_segs_data) == 0:
        ...
        label_segs_data_np = np.empty(0).reshape(0, seg_len)
    else:
        pose_segs_data_np = np.concatenate(pose_segs_data, axis=0)
        score_segs_data_np = np.concatenate(score_segs_data, axis=0)
        label_segs_data_np = np.concatenate(label_segs_data, axis=0)

    global_pose_data_np = np.concatenate(global_pose_data, axis=0)
    del pose_segs_data
    # del global_pose_data
    if ret_keys:
        return pose_segs_data_np, pose_segs_meta, person_keys, global_pose_data_np, global_pose_data, score_segs_data_np, label_segs_data_np
    else:
        return pose_segs_data_np, pose_segs_meta, global_pose_data_np, global_pose_data, score_segs_data_np, label_segs_data_np
...
def single_pose_dict2np(person_dict, idx):
    ...
    for key in single_person_dict_keys:
        ...
        sing_label_np.append(single_person[key]['label'])
    sing_pose_np = np.stack(sing_pose_np, axis=0)
    sing_scores_np = np.stack(sing_scores_np, axis=0)
    return sing_pose_np, sing_pose_meta, single_person_dict_keys, sing_scores_np, np.array(sing_label_np)
...
def split_pose_to_segments(single_pose_np, single_pose_meta, single_pose_keys, start_ofst=0, seg_dist=6, seg_len=12,
                           scene_id='', clip_id='', single_score_np=None, dataset="ShanghaiTech", sing_label_np=None):
    ...
    pose_label_np = np.empty([0, seg_len])
    pose_segs_meta = []
    ...
    for seg_ind in range(num_segs):
        ...
        if is_seg_continuous(single_pose_keys_sorted, start_key, seg_len):
            ...
            curr_label = sing_label_np[start_ind:start_ind + seg_len].reshape(1, seg_len)
            pose_segs_np = np.append(pose_segs_np, curr_segment, axis=0)
            pose_score_np = np.append(pose_score_np, curr_score, axis=0)
            pose_label_np = np.append(pose_label_np, curr_label, axis=0)
            if dataset == "UBnormal":
    ...
    return pose_segs_np, pose_segs_meta, pose_score_np, pose_label_np
