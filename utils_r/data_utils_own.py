...
def apply_pose_transform(pose, trans_mat):
    ...
    conf = np.expand_dims(pose[1], axis=0)
...
def normalize_pose(pose_data, **kwargs): # 24 18 2 or 3
    """
    Normalize keypoint values to the range of [-1, 1]
    :param pose_data: Formatted as [N, T, V, F], e.g. (Batch=64, Frames=12, 18, 3)
    :param vid_res:
    ...
    :return:
    """
    vid_res = kwargs.get('vid_res', [3840,2160]) # 1920 1080 # 3840 2160
    ...
    # vid_res_wconf = vid_res + [1] # pose是2维的
    norm_factor = np.array(vid_res)
    pose_data_normalized = pose_data / norm_factor  # 更加稳定
    pose_data_centered = pose_data_normalized
    if symm_range:  # Means shift data to [-1, 1] range
        pose_data_centered[..., :2] = 2 * pose_data_centered[..., :2] - 1
    ...
    if pose_data_zero_mean.shape[-1] == 3:
        pose_data_zero_mean[..., :3] = (pose_data_centered[..., :3] - pose_data_centered[..., :3].mean(axis=(1, 2))[:, None, None, :]) / pose_data_centered[..., :3].std(axis=(1, 2))[:, None, None, None]
    else:
