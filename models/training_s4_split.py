...
class Trainer:
    ...
    def train(self, log_writer=None, clip=100):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, dataarr in enumerate(pbar):
                try:
                    norm, abnorm = dataarr[0], dataarr[1]
                    data = [data.to(self.args.device, non_blocking=True) for data in norm]
                    data_ab = [data_ab.to(self.args.device, non_blocking=True) for data_ab in abnorm]
                    # score = data[-3].amin(dim=-1)
                    label_ab = data_ab[-1] # 256 的 float 64 1 ---> 正常数据 都是1  ### 有label 了
                    ...
                    samp_ab = samp_ab.permute(0,2,1,3).reshape(-1, self.seg_len, 36).float()

                    input =  samp[:,0:self.input_frame,:] # 预测
                    input_ab = samp_ab[:,0:self.input_frame,:] # 预测
                    # input =  samp[:,self.seg_len-self.input_frame:self.seg_len,:] # 反预测
                    pred = self.model(input) # sample 256 2 24 18  --》256 144
                    pred_ab = self.model(input_ab) # sample 256 2 24 18  --》256 144

                    if pred is None and pred_ab is None:
                        continue
                    # if self.args.model_confidence: # 不同置信度
                    #     pred = pred * score
                    
                    p_truth = samp[:,self.input_frame:self.seg_len,:].reshape(-1, (self.seg_len-self.input_frame )* 36)
                    p_truth_ab = samp_ab[:,self.input_frame:self.seg_len,:].reshape(-1, (self.seg_len-self.input_frame )* 36)
                    # p_truth = samp[:,0:self.seg_len-self.input_frame,:].reshape(-1, (self.seg_len-self.input_frame )* 36) # 反预测
                    # loss1 = self.lossr(pred, p_truth)
                    # loss2 = self.lossr(pred_ab, p_truth_ab)
                    # 指数

                    # losses = torch.exp(loss1) - torch.exp(loss2)
                    # losses = loss1 - loss2

                    # ### one hot ?
                    # losses = self.cross_entropy(loss1,loss2)
                    # losses.backward()
                    # #### 无监督的训练法：
                    ...
                    # orlu_dis_ab = self.lossr(pred_ab, p_truth_ab).reshape(-1, (self.seg_len-self.input_frame), 36)
                    # orlu_dis_ab = torch.mean(orlu_dis_ab, dim=-1) # 256 x 4
                    # label_4 = label_ab[::,-4:] # 
                    ...
                    # dis_pred_ab = self.model_unsupervised(input_ab)
                    # orlu_dis_dis_ab = self.lossr(dis_pred_ab, p_truth_ab).reshape(-1, (self.seg_len-self.input_frame), 36)
                    # orlu_dis_dis_ab = torch.mean(orlu_dis_dis_ab, dim=-1) # (0到1 之间)
                    ...
                    # label_4_dis = label_ab[::,-4:].float() +label_ab[::,-4:].float() * orlu_dis_dis_ab + orlu_dis_dis_ab  # 2 目前最好 lr 5e5
